...
import { ORANGE, PINK, YELLOW, YELLOW_SOFT } from "../lib/palette.js";
...
function preview(cosmetic) {
  ...
  if (cosmetic.kind === "star_color") {
    const colors =
      payload.mode === "era"
        ? [YELLOW_SOFT, YELLOW, ORANGE, PINK]
        : payload.mode === "difficulty"
          ? [PINK, YELLOW_SOFT, YELLOW, "#ffffff"]
  ...
  if (cosmetic.kind === "glow") {
    ...
    return el(
      ...
      el("span", {
        class: "swatch-dot",
        style: `background:${YELLOW};box-shadow:0 0 ${Math.round(4 + scale * 12)}px ${Math.round(
          1 + scale * 5
        )}px rgba(244,193,74,${0.15 + scale * 0.18})`,
      })
    );
  }
  ...
  const dash = payload.dash ? `repeating-linear-gradient(90deg,${YELLOW} 0 5px,transparent 5px 10px)` : YELLOW;
  return el(
    "div",
    { class: "swatch-row" },
    el("span", {
...
export async function shopView(outlet) {
  ...
  async function renderWallet() {
    try {
      ...
      const wallet = store.wallet || (await api.wallet());
      walletHost.replaceChildren(
        ...
        el(
          ...
          el("div", { class: "stat", style: `color:${YELLOW_SOFT}` }, wallet.gold.toLocaleString()),
