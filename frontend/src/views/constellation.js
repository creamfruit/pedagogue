...
import { ERA_COLOR, LINK_TYPE_COLOR, NEUTRAL, PINK, STAR_DEFAULT, YELLOW, difficultyColor } from "../lib/palette.js";

const LINK_COLORS = LINK_TYPE_COLOR;
...
const ERA_COLORS = ERA_COLOR;

const CUSTOM_COLOR = PINK;
const UNVERIFIED_COLOR = NEUTRAL;
const FROZEN_COLOR = "#cdd3dc";
...
const MIN_DOT = 2.2;
const MAX_DOT = 22;
const SCALE_CURVE = 3.1;

function expScale(t, min, max, k = SCALE_CURVE) {
  const clamped = Math.min(Math.max(t, 0), 1);
  return min + (max - min) * (Math.exp(k * clamped) - 1) / (Math.exp(k) - 1);
}
...
export async function constellationView(outlet) {
  ...
  const detail = el(
    ...
    el("p", { class: "faint", style: "margin:0" }, "Tap a star for the piece, tap a line for why it connects. Drag a star to send it drifting.")
  );
  ...
  function starColor(node) {
    ...
    if (starStyle.mode === "difficulty") return difficultyColor(difficulty);
    return ERA_COLORS[node.era] || STAR_DEFAULT;
  }

  const nodes = graph.nodes.map((node) => {
    const t = Math.min(Math.max((node.difficulty ?? 50) / 100, 0), 1);
    const dot = expScale(t, MIN_DOT, MAX_DOT);
    return {
      ...node,
      dot,
      radius: dot + 6,
  ...
  function openDetail(node) {
    ...
    const statusPills = [
      ...
      node.decay >= 0.85 ? el("span", { class: "pill", style: `color:${FROZEN_COLOR}` }, "frozen · needs maintenance") : null,
    ];
    const actions = [
