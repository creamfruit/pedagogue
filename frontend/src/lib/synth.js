...
function pitchToFrequency(pitch) {
  ...
  const semitone = NOTE_SEMITONE[letter] + (sharp ? 1 : 0);
  ...
  notation.measures.forEach((measure, measureIndex) => {
    ...
    measure.notes.forEach((note) => {
      ...
      pitches.forEach((pitch) => {
        ...
        gain.gain.linearRampToValueAtTime(0.16, time + 0.012);
        gain.gain.exponentialRampToValueAtTime(0.0001, time + duration * 0.94);
