...
import { PINK } from "../lib/palette.js";
...
export function pieceCoreStats(overview) {
  ...
  return el("section", { class: "panel core-stats" }, el("div", { class: "meta-grid" }, ...rows));
}
...
export function pieceOverview(overview, { heading = true } = {}) {
  ...
  const badges = [
    overview.is_custom ? el("span", { class: "pill", style: `color:${PINK}` }, "custom") : null,
  ...
  wrap.append(
    el(
      ...
      metaRow("Difficulty band", overview.difficulty_score != null ? `${overview.difficulty_score} · ${overview.difficulty_band}` : null),
